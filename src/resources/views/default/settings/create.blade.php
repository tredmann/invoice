...
@section('content')
    ...
    <section id="header_section">
        <h1 class="title is-size-5 is-uppercase">{{ __('settings.add') }}</h1>
    </section>

    <section id="content_section" class="mt-5 box pb-1 p-0">
        <div id="content_head">
            <h1>{{ __('settings.add') }}</h1>
        </div>

        <form id="content_form" class="p-4" method="POST" action="{{$tenant->route('settings.store')}}">
        @csrf

...
@section('content')
    ...
    <section id="header_section">
        <h1 class="title is-size-5 is-uppercase">{{ __('lineItems.edit') }}</h1>
    </section>

    <section id="content_section" class="mt-5 box pb-1 p-0">
        <div id="content_head">
            <h1>{{ __('lineItems.edit') }}</h1>
        </div>

        <form id="content_form" class="p-4" method="POST" action="{{ $tenant->route('lineItems.update', ['lineItem' => $lineItem]) }}">
        @csrf
        @method('PATCH')
        ...
        <!-- Quantity -->
            <div class="field">
                ...
                <div class="control">
                    <input id="quantity" name="quantity" type="number"
                           ...
                           value="{{old('quantity') ?? str_replace('.', ',', $lineItem->quantity)}}"
                           min="0"
                           step=".01"
                           max="999999999999"
                           required>
            ...
            <div class="field">
                ...
                <div class="control">
                    <input id="price_each" name="price_each" type="number"
                           class="input"
                           onchange="toFloat(event)"
                           value="{{old('price_each') ?? str_replace('.', ',', money($lineItem->price_each))}}"
                           min="0"
                           step=".01"
                           max="9999999"
                           required>
            ...
            <input type="hidden" name="currency" value="{{$lineItem->currency}}">
                    ...
                    <div class="select is-fullwidth">
                        <select id="tax_rate" name="tax_rate" required>
                            @foreach(\App\Models\Money::DE_TAX_RATES as $tax_rate)
                                <option value="{{$tax_rate}}" @if(old('tax_rate') ? old('tax_rate') === $tax_rate : $tax_rate === $lineItem->tax_rate) selected @endif>@tax($tax_rate)</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="select is-fullwidth">
                        <select id="unit" name="unit" required>
                            @foreach(\App\Enums\UnitCode::cases() as $code)
                                <option value="{{ $code->value }}" {{ (old('unit') ?? $lineItem->unit->value) === $code->value ? 'selected' : '' }}>
                                    {{ $code->label() }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="field">
                ...
                <div class="control">
                    <input id="detail" name="detail" type="text"
                           class="input"
                           value="{{old('detail') ?? $lineItem->detail}}"
                           required>
                </div>
            </div>
            ...
            <div class="field">
                ...
                <div class="control">
                        <textarea id="detail_plus" name="detail_plus"
                                  ...
                                  >{{old('detail_plus') ?? $lineItem->detail_plus}}</textarea>
                </div>
...
<?php $sidebarActive = true; $sidebar = 'default.customers.components.customersSidebar'; $customer=$lineItem->invoice->customer?>

...
@section('content')
    <section id="header_section">
        <h1 class="title is-size-5 is-uppercase">{{ __('masterLineItems.add') }}</h1>
    </section>

    <section id="content_section" class="mt-5 box pb-1 p-0">
        <div id="content_head">
            <h1>{{ __('masterLineItems.add') }}</h1>
        </div>
        <form id="content_form" class="p-4" method="POST" action="{{$tenant->route('masterLineItems.store')}}">
            @csrf

            <input class="form--input" type="hidden" name="referrer_url" value="{{url()->previous()}}">

            <input class="form--input" type="hidden" name="master_invoice_id" value="{{$masterInvoice->id}}">
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <input id="currency" name="currency" type="text"
                               class="input"
                               value="{{$masterInvoice->currency}}"
                               disabled
                               required>
                    </div>
                </div>

                <input type="hidden" name="currency" value="{{$masterInvoice->currency}}">
        @endif
            ...
            <div class="field">
                ...
                <section id="button_section" class="mt-6">
                    <div class="mt-5 is-flex is-justify-content-space-between is-align-items-center">
                        <!-- Cancel -->
                        <a href="{{ $tenant->route('masterInvoices.masterLineItems', ['masterInvoice' => $masterInvoice]) }}"
                        ...
                        <button name="submit"
                                value="continue"
                                type="submit"
                                class="button is-danger is-radiusless">{{ __('masterLineItems.add_more') }}</button>

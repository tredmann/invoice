<html>
<head http-equiv="Content-Type" content="text/html; charset=utf-8" >
    <style>

        @page { margin: 50px 50px 50px; }

        body {
            margin-bottom: 15px;
            font-size: medium;
        }

        header {

            font-size: 10px;
        }

        footer {
            position: fixed;
            bottom: 0;
            left: 0;
            right: 0;

            font-size: 9px;
        }

        table {
            width: 100%;
        }

        main table {
            border-collapse: collapse;
        }

        main #line_items th {
            border-bottom: 1px solid black;
            text-align: left;
            font-size: medium;
        }

        main #line_items td {
            border-bottom: 1px solid black;
            padding-bottom: 5px;
            padding-top: 5px;
            font-size: small;
            vertical-align: top;
        }

        header #placeholder_image {
            height: 75px;
            width: auto;
            max-width: 150px;
        }

        div {
            page-break-inside: avoid;
        }

        p {
            margin: 2px;
        }

        .thick {
            font-weight: bold;
        }

    </style>
</head>
<body>
<footer style='margin-left: -20px; margin-right: -20px'>
    <table style='border-collapse: separate; border-spacing: 20px; width: 100%;
    table-layout: fixed;'>
        <tr>
            <td style="width:25%; text-align: left; vertical-align: top; text-transform: uppercase; border-top: 1px solid black;
            border-bottom: 1px solid black;">
                <div><strong>{{$generalInfo->name}}</strong></div>
                <div>{{$generalInfo->street}}</div>
                <div>{{$generalInfo->postal}} {{$generalInfo->city}}</div>
            </td>
            <td style="width:25%; vertical-align: top; text-transform: uppercase; border-top: 1px solid black;
            border-bottom: 1px solid black;">
                <div><strong>Kontakt</strong></div>
                <div>Tel {{$generalInfo->telephone}}</div>
                <div>Fax {{$generalInfo->fax}}</div>
                <div>{{$generalInfo->email}}</div>
                <div>{{$generalInfo->homepage}}</div>
            </td>
            <td style="width:25%; vertical-align: top; text-transform: uppercase; border-top: 1px solid black;
            border-bottom: 1px solid black;">
                <div><strong>Geschäftsführende<br/>Gesellschafter</strong></div>
                <div>Silvio Schobinger</div>
                <div>Mario Schobinger</div>
                <div>AG Charlottenburg</div>
                <div>HRA 50203 B</div>
            </td>
            <td style="width:25%; vertical-align: top; text-transform: uppercase; border-top: 1px solid black;
            border-bottom: 1px solid black;">

                <div><strong>{{ $legalInfo->bank_institute}}</strong></div>
                <div>IBAN: {{$legalInfo->iban}}</div>
                <div>BIC: {{$legalInfo->swift_bic}}</div>
                <div>&nbsp;</div>
                <div>St.-Nr. {{$legalInfo->tax_no}}</div>
                <div>Ust-ID {{$legalInfo->vat_no}}</div>


            </td>
        </tr>
    </table>
</footer>
<main>

    <header>
        <table>
            <tr>
                <td style="text-align: right; vertical-align: top">
                    <div class="vert-tenant-info">
                        <img style='width: 6cm;' src="data:image/png;base64,"/>
                    </div>
                </td>
            </tr>
            <tr><td>
                    <div class="hor-tenant-info">
                        <div style="text-transform: uppercase"><strong>{{$generalInfo->name}}</strong>  {{$generalInfo->street}}  {{$generalInfo->postal}} {{$generalInfo->city}}</div>
                    </div>
                </td>
            </tr>
        </table>
    </header>

    <div id="info" style="margin-bottom: 50px">
        <div id="customer_info" style="margin-top: 10px; margin-bottom: 50px;">
            <div id="customer_info">
                <p>{{$customer->company}}</p>
                <p>{{$customer->name}}</p>
                <p>{{$customer->street}}</p>
                <p>{{$customer->postal}} {{$customer->city}}</p>
            </div>
        </div>
        <div id="invoice_info" >
            <table>
                <tr>
                    <td style="text-align: left; vertical-align: top">
                        <p class="thick">Rechnungs-Nr.
                            {{$invoice->invoice_no ?: sprintf('(%s)', $uniqueNumber::predictedNumber($invoice, 4))}}
                        </p>
                    </td>
                    <td style="text-align: right; vertical-align: top">
                        <p>{{ __('translate.date') }}: {{now()->tz('CET')->format('d.m.Y')}}</p>
                        <p>{{ __('attributes.customer_no') }}: {{$customer->customer_no}}</p>
                        <p>Rechnungs-Nr.:
                            {{$invoice->invoice_no ?: sprintf('(%s)', $uniqueNumber::predictedNumber($invoice, 4))}}
                        </p>

                    </td>
                </tr>
            </table>
        </div>
    </div>

    <table id="line_items">
        <thead>
        <tr>
            <th style="width: 1.5cm">{{ __('lineItems.positionShort') }}</th>
            <th style="width: 50%">{{ __('lineItems.details') }}</th>
            <th style="text-align: right">{{ __('attributes.without_tax') }}</th>
        </tr>
        </thead>
        <tbody>
        @foreach($invoice->lineItems->sortBy('created_at') as $lineItem)
            <tr>
                <td >{{$loop->iteration}}</td>
                <td>
                    <p class="thick">{{$lineItem->detail}}</p>
                    <p>{!! nl2br($lineItem->detail_plus) !!}</p>
                </td>
                <td style="text-align: right; width: 2cm">@money($lineItem->without_tax, $lineItem->currency)</td>
            </tr>
        @endforeach
        </tbody>
    </table>

    <div id="calculations">
        <div id="interim_calc" style="margin-top: 10px; margin-bottom: 5px">

            <table>
                <tr>
                    <td class='thick'>
                        {{ __('attributes.total_without_tax') }}
                    </td>
                    <td style="text-align: right; vertical-align: top" class='thick'>
                        <p>
                            @money($invoice->total_without_tax, $invoice->currency)
                        </p>
                    </td>
                </tr>
            </table>


            <table>
                @foreach($invoice->getTaxDistribution() as $taxRate => $tax)
                    <tr>
                        <td>
                            <p>
                                {{$taxRate}}% (@money($tax['without_tax'], $invoice->currency))
                            </p>
                        </td>
                        <td style="text-align: right; vertical-align: top">
                            <p>
                                @money($tax['tax'], $invoice->currency)
                            </p>
                        </td>
                    </tr>
                @endforeach
            </table>

        </div>

        <hr class="medium-thin">

        <div id="final_calc" style="margin-top: 5px">
            <table>
                <tr>
                    <td class="thick">
                        {{ __('attributes.total_with_tax') }}
                    </td>
                    <td style="text-align: right; vertical-align: top" class='thick'>
                        <p>
                            @money($invoice->total_with_tax, $invoice->currency)
                        </p>
                    </td>
                </tr>
            </table>

        </div>
    </div>

    <div id="payment">
        <div id="payment_instruction" style="margin-top: 30px">

            <p>Wir bitten um Zahlung des vollen Rechnungsbetrages in Höhe von @money($invoice->total_with_tax, $invoice->currency) unter Angabe der Rechnungs-Nr. {{$invoice->invoice_no ?: sprintf('(%s)', $uniqueNumber::predictedNumber($invoice, 2))}} auf das unten
                aufgeführte Bankkonto.</p>
            <p><strong>Bitte beachten Sie, dass sich unsere Bankverbindung für Zahlungen von Stromrechnungen geändert hat. Die Bankverbindung für Mietzahlungen bleibt unverändert.</strong></p>
        </div>

        <div id="payment_details" style="margin-top: 10px">
            <p>{{ __('invoicePdf.reason_for_payment') }}:
                {{$invoice->invoice_no ?: sprintf('(%s)', $uniqueNumber::predictedNumber($invoice, 4))}}
            </p>
            <p>{{ __('attributes.bank_institute') }}: {{$legalInfo->bank_institute}}</p>
            <p>{{ __('attributes.iban') }}: {{$legalInfo->iban}}</p>
            <p>{{ __('attributes.swift_bic') }}: {{$legalInfo->swift_bic}}</p>
            <p>Überweisungsbetrag: @money($invoice->total_with_tax, $invoice->currency)</p>
        </div>

        <div style="margin-top: 30px">
            <p>Der Rechnungsbetrag ist sofort nach Erhalt der Rechnung zur Zahlung fällig.</p>
            <p>&nbsp;</p>
        </div>
    </div>

    <script type="text/php">
        if (isset($pdf))
        {
            $x = 267.5;
            $y = 815;
            $text = "{{ __('invoicePdf.page') }} {PAGE_NUM} {{ __('invoicePdf.of') }} {PAGE_COUNT}";
            $font = null;
            $size = 8;
            $color = array(0,0,0);

            $h = $pdf->get_height();
            $w = $pdf->get_width();



            $word_space = 0.0;  //  default
            $char_space = 0.0;  //  default
            $angle = 0.0;   //  default
            $pdf->page_text($x, $y, $text, $font, $size, $color, $word_space, $char_space, $angle);
        }
    </script>
</main>
</body>
</html>

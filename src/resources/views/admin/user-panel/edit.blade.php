...
@section('content')
    ...
    <section id="header_section">
        <h1 class="title is-size-5 is-uppercase">{{ __('admin/user-panel.edit') }}</h1>
    </section>


    <section id="content_section" class="mt-5 box pb-1 p-0">
        <div id="content_head">
            <h1>{{ __('admin/user-panel.edit') }}</h1>
        </div>

        <form id="content_form" class="p-4" method="POST" action="{{route('admin.user-panel.update', ['user' => $user])}}">
        @csrf
        @method('PATCH')
            ...
            <div class="field">
                ...
                <div class="control">
                    <input id="password" name="password" type="password"
                           class="input"
                           value="{{old('password')}}">
                    <small>{{ __('admin/user-panel.password_small') }}</small>
                </div>
            </div>
